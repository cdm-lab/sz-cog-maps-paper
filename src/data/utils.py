...
def create_sub_dict(subnum, df, num_first_stage, len_of_study):
    """
    ...
    if (
        len(sub_df) < len_of_study
    ):  # checks to see if subject has completed all trials (if they don't respond they are still
        # counted towards length) #TODO figure out some way to replace this with a study specific number
    ...
    if (
        len({1, 2} - set(sub_df["response1"].unique())) != 0
    ):  # checks to see if subject hasn't responded with both keys
        return "is a bad subject"
    if sub_df["high_arm"].unique()[0] == 1:
        sub_dict["high_arm"] = "inside"
    else:
        sub_dict["high_arm"] = "outside"
    if num_first_stage == 4:
        envs = ["desert", "forest", "library", "restaurant"]
    elif num_first_stage == 2:
        envs = ["desert", "forest"]
    object_list = []

    for i in range(1, num_first_stage + 1):
        sub_dict[envs[i - 1]] = (
            sub_df[sub_df["state1"] == i].iloc[0][["stim_left", "stim_right"]].values
        )
    ...
    sub_dict["reward_1"] = list(
        dict.fromkeys(rew_1)
    )  # grabs every other index and is sorted by desert,forest,library,restaurant
...
def fill_slider_matrix(df, flat_list):
    len_diag = len(flat_list)
    sorted_slider_matrix = np.zeros((len_diag, len_diag))
    for i in range(len(df)):
        object_1 = df.iloc[i]["stim1"][8:-4]
        object_2 = df.iloc[i]["stim2"][8:-4]
        idx1 = flat_list.index(object_1)
        idx2 = flat_list.index(object_2)
        sorted_slider_matrix[idx1, idx2] = df["response"].iloc[i]
    sorted_slider_matrix += sorted_slider_matrix.T
    sorted_slider_matrix += np.eye(len_diag) * 100
    return sorted_slider_matrix / 2


def create_2stage_sorted_sliders(sub_dict, subnum, df):
    """
    creates sorted sliders for when there are only 2 first_stage states
    """
    isbad = False
    ...
    object_ordered_list = [rew_1_obj_list]
    ...
    object_ordered_list.append(["object_07", "object_08"])
    flat_list = [item for sublist in object_ordered_list for item in sublist]
    pre_sorted_identity = fill_slider_matrix(presliders, flat_list)

    post_sorted_identity = fill_slider_matrix(postsliders, flat_list)

    return [pre_sorted_identity, post_sorted_identity], isbad
...
def create_state2_sorted_sliders(sub_dict, subnum, df):
    ...
    rew1 = sub_dict[subnum]["reward_1_object"]
    rew2 = sub_dict[subnum]["reward_2_object"]
    second_ordered_list.append(
        list(sub_dict[subnum]["reward_1"])
    )  # have to reverse order so high arm is first
...
def create_model_matrices_old():  # TODO delete this function
    """
    ...
    post_state1_sort[9, 4] = 0.5
    post_state1_sort[4, 9] = 0.5
...
def create_model_matrices():
    ...
    wrong_sort[4, 2] = 1
    wrong_sort[2, 4] = 1
    return [state1_sort, l_sort, o_sort, wrong_sort]
